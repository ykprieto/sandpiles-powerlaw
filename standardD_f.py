...
def grafica_all_sizes(xPoint,y_50,y_100,y_200,y_300,y_500,y_1000,case):
	...
	plt.xticks(dominio)
	if case==0:		#case=0 is a power enteger
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_DE/fSD_alpha"+str(int(alpha))+"_coef"+str(coef)+".pdf")
	else:			#other case is an alpha-root	
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_DE/fSD_alpharoot"+str(int(alpha))+"_coef"+str(coef)+".pdf")
	plt.show()
	plt.close()
#----------------------------------------------------------------------------------------------------	

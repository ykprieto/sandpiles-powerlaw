...
#----------------------------------------------------------------------------------------------------
#calcula funcion promedio fijando n
def function_f_k(m,alpha,vector_average):		#function depends of k & fix n
	yPoint=[]
	for i in range (0,len(vector_pts)):
		yPoint=yPoint + [round(vector_average[vector_size.index(m)][i]/(m*math.pow(vector_pts[i],alpha)), 9)]
	return yPoint
#----------------------------------------------------------------------------------------------------
def function_fR_k(m,alpha,vector_average):		#function depends of k & fix n
	yPointR=[]	
	for i in range (0,len(vector_pts)):
...
def graficar(xPoint,yPoint,case):
	...
	plt.xticks(dominio)
	if case==0:		#case=0 is a power enteger
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_mean/f_alpha"+str(alpha)+ "_size"+str(n) +"_coef"+str(coef)+".pdf")
	else:			#other case is an alpha-root	
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_mean/f_alpharoot"+str(alpha)+"_size"+ str(n) +"_coef"+str(coef)+".pdf")
	plt.close()
	#plt.show()
def grafica_all_sizes(xPoint,y_50,y_100,y_200,y_300,y_500,y_1000,case):
	...
	plt.xticks(dominio)
	if case==0:		#case=0 is a power enteger
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_mean/fmean_alpha"+str(int(alpha))+"_coef"+str(int(coef))+".pdf")
		#plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_mean/fDE_alpha"+str(alpha)+"_coef"+str(coef)+".pdf")
	else:			#other case is an alpha-root	
		plt.savefig("/home/ykprieto/Dropbox/Algoritmos/Graficas_fun_mean/fmean_alpharoot"+str(int(alpha))+"_coef"+str(int(coef))+".pdf")

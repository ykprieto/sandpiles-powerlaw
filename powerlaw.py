...
# Version     :
# Copyright   :
# Description : This program computes a histogram of power law from avalanches calculated.
...
pts=sys.argv[2]
NN=int(sys.argv[3])
name_file="/home/ykprieto/Dropbox/Algoritmos/power_law/bueno/power"+size+"_"+pts+"w.txt"
...
def histogram(xx,yy,yyy,sw):			#log-log scale
	...
	plt.savefig("./imagenes_PL/PL"+size +"x"+pts+"_sw"+ str(sw)+".pdf")
	plt.show()
	plt.clf()
	if sw==0:		
...
def particion(vector):	
	...
	yy=[]
	zz=[]
	archi=open("/home/ykprieto/Dropbox/Algoritmos/power_law/tablas_PL/FreqOriginal"+size+"_"+pts+".txt",'w')
	for i in range(0,bi):
		if freq[i]>0:
			xx=xx+[bins[i]]
			yy=yy+[freq[i]]
...
def tabla_freq(vector):
	...
	archi=open("/home/ykprieto/Dropbox/Algoritmos/power_law/tablas_PL/Freq"+size+"_"+pts+".txt",'w')
	linea=""
...
archi=open("/home/ykprieto/Dropbox/Algoritmos/power_law/imagenes_PL/criticalexponents.txt",'a')
...
#figure without log-log scale
	#xx=np.exp(xx)
	#yy=np.exp(yy)
	#yyy=np.exp(c)*(np.power(xx,m))	
	#plt.plot(xx, yyy, 'r', marker='.',ls='',label='Fitted line')
	#plt.plot(xx,yy,'b',marker='.',ls='',label='Experimental values')
	#plt.legend(loc='best',ncol=2)
	#plt.savefig("./imagenes_PL/PL"+size +"x"+pts+"_subset"+ str(NN)+"ORIGINAL.pdf")
	#plt.show()

...
//               parallel fashion
//==============GUIDE==========================================================================================================
// COMPILE  : mpicxx -std=c++0x sandpile.cpp -o sandpile_usual
// RUN      : ./sandpile_usual size_x size_y pts seed part_x part_y	no parallel then part_x = part_y = 1
// REMARKS  : Change the path of saved pictures in "ruta".
//	      Add the folder: images_sand to save the info of the picture.
//=============================================================================================================================
...
void writeout(int num_ptos,int num, int varRandom,const subgrid& s)
{
    std::string ruta = "",ruta_i;
    ruta="/home/ykprieto/Descargas/sandpiles-powerlaw-master";
    ruta_i=ruta+"/images_sand/gridSand_"+static_cast<std::ostringstream*>(&(std::ostringstream() << num_ptos))->str()+ "pts_"+static_cast<std::ostringstream*>(&(std::ostringstream() << num))->str() + "size_"+ static_cast<std::ostringstream*>(&(std::ostringstream() << varRandom))->str()+"_experiment.dat";
    string path(ruta_i);
...
int main(int argc, char **argv) {
    ...
    if (world_rank==0)
    {
        ...
        writeout(num_ptos,num_grid, varRandom,total);
    }
    else
    {
    ...
    cout<<"El tiempo de ejecucion fue: "+static_cast<std::ostringstream*>(&(std::ostringstream() << t1))->str()+"\n";
    return 0;
}
